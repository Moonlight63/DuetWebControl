<template>
  <grid-panel v-bind="$attrs" class="d-flex flex-column pa-2">
      ...
      <v-window ref="window" vertical class="flex-grow-1">
          ...
          >
            ...
            >
                ...
                <v-list-item v-for="i in itemsSplit[n-1].length" :key="i" two-line @click="$emit('click')">
                    <autosize-text class="fill-height" innerClass="" :icon="itemsSplit[n-1][i-1].icon" :fitWidth="false" />
                    <v-list-item-content>
                        <v-list-item-title>{{ itemsSplit[n-1][i-1].title }}</v-list-item-title>
