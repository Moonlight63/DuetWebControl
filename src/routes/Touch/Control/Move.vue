			...
			<div class="component d-flex flex-column flex-grow-1" style="min-height: 0">
				<v-container style="overflow: auto">
					<v-row justify="center">
                        ...
                        <touch-nav-btn label="Distance" icon="mdi-map-marker-distance"></touch-nav-btn>
                    ...
                    <v-row justify="center">
                        <touch-code-btn 
                        :code="`M120\nG91\nG1 X-${moveSteps('X')[axisStepIndex['X']]} F${moveFeedrate}\nG90\nM121`" no-wait
                        label="X-" 
                        ...
                        <touch-code-btn 
                        code="G28" label="Home All"
                        :color="(unhomedAxes.length !== 0) ? 'warning' : 'primary'"
                        :icon="(unhomedAxes.length !== 0) ? 'mdi-home-alert' : 'mdi-home-circle-outline'">
                        </touch-code-btn>
                        ...
                        <touch-nav-btn 
                        @click="setDistanceIndex('Y')" 
                        :label="'Y ' + axisStepIndex['Ylabel']" 
                        @contextmenu.prevent="showMoveStepDialog('Y', axisStepIndex['Y'])"
                        icon="mdi-map-marker-distance"
                        ></touch-nav-btn>
					</v-row>
                    <v-row justify="center">
                        <touch-code-btn 
                        :code="`M120\nG91\nG1 Z-${moveSteps('Z')[axisStepIndex['Z']]} F${moveFeedrate}\nG90\nM121`" no-wait 
                        label="Z-" 
                        ...
                        <touch-nav-btn 
                        @click="setDistanceIndex('Z')" 
                        :label="'Z ' + axisStepIndex['Zlabel']" 
                        @contextmenu.prevent="showMoveStepDialog('Z', axisStepIndex['Z'])"
                        icon="mdi-map-marker-distance">
                        </touch-nav-btn>
					</v-row>
				</v-container>
