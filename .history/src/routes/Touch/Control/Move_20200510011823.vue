			...
			<div class="component d-flex flex-column flex-grow-1" style="min-height: 0">
				<v-container style="overflow: auto">
					<v-row justify="center">
                        ...
                        <touch-nav-btn to="/touch/control" label="Distance X" icon="mdi-map-marker-distance"></touch-nav-btn>
					</v-row>
                    <v-row justify="center">
                        ...
                        <touch-nav-btn @click="setDistanceIndex('Y')" :label="moveSteps('Y')[axisStepIndex['Y']]" icon="mdi-map-marker-distance"></touch-nav-btn>
					</v-row>
                    <v-row justify="center">
                        ...
                        <touch-nav-btn to="/touch/control" label="Distance Z" icon="mdi-map-marker-distance"></touch-nav-btn>
					</v-row>
				</v-container>
			</div>
		</v-col>
...
export default {
    ...
    mounted() {
        this.displayedAxes.forEach(axis => {
            this.axisStepIndex[axis.letter] = this.moveSteps(axis.letter)[this.numMoveSteps.length-1];
        });
	...
	methods: {
        ...
        setDistanceIndex(axisLetter) {
            if (this.axisStepIndex[axisLetter] + 1 > this.numMoveSteps.length - 1)
                this.axisStepIndex[axisLetter] = 0;
            else
