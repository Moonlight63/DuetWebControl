		...
		<v-card class="d-flex flex-column">
			<v-app-bar flat dark color="primary" class="flex-grow-0 flex-shrink-1">
				<v-row class="d-flex">
					...
					<v-col class="d-flex flex-grow-1 justify-center">
						<v-btn dark text class="ml-auto" :disabled="uiFrozen" :loading="doingCode" @click="focusInput">
							<v-icon class="mr-1">mdi-send</v-icon> Test
						</v-btn>
					</v-col>
			...
			<div class="flex-grow-1">
				<v-card flat class="mx-auto mt-10" style="width: 50%">
					<!-- <v-card-text> -->
						<v-combobox ref="input" :solo=true hide-details :disabled="uiFrozen" :placeholder="$t('input.code.placeholder')" autofocus
			...
			<v-container class="d-flex py-0 justify-center">
				<touch-keyboard ref="keyboard" @onChange="onKeyboardChange" @onKeyPress="onKeyPress" :input="code" :theme="keyboardTheme" />			
			</v-container>
		</v-card>
	</v-dialog>
...
export default {
	...
	methods: {
		...
		focusInput() {
			...
			setTimeout(() => this.$refs.input.activateMenu(), 50);
			//setTimeout(() => this.$refs.input.$refs.input.focus(), 50);
		},
		onKeyboardChange(input) {
			...
			this.$refs.input.activateMenu();
			this.$nextTick(() => {
				const inputTemp = this.$refs.input.$el.querySelector('input');
				inputTemp.focus();
			});
		...
		selectItem(e) {
			if (this.selectedItem !== '') {
				if(this.$refs.keyboard != null){
					this.$refs.keyboard.resetKeyboard();
