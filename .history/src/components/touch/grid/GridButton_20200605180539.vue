            ...
            <div v-if="rows == 1" class="d-flex fill-height" style="width:100%;">
                <div ref="iconTest" class="fill-height">
                    <span ref="innerSpanIcon" class="iconFitted text-center d-inline-block">
                        <v-icon :size="iconSize" class="mb-0 flex-shrink-1 notransition" :color="iconColor" left style="width:auto; height:auto">{{ icon }}</v-icon>
                    </span>
                </div>
    ...
    methods: {
        ...
        updateIconSize() {
            ...
            while (low <= high && count < 21) {
                ...
                if(innerSpan.clientHeight <= originalHeight){
                    size = mid;
                    low = mid + 1;
                } else {
                    high = mid - 1;
                ...
                console.log('Span Scroll Height: ' + innerSpan.clientHeight);
            ...
            this.$nextTick(console.log(JSON.stringify(innerSpan)));
        },
