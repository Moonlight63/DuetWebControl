    ...
    <grid-panel ref="elm" :columns='columns || 2' :rows='rows || 2' elevation="0">
        ...
        >
            ...
            <div v-if="rows == 1" class="d-flex fill-height" style="width:100%;">
                <div ref="iconTest" class="fill-height pa-2">
                    <!-- <span ref="innerSpanIcon" class="iconFitted text-center d-inline-block"> -->
                        <v-icon ref="icon" class="mb-0 flex-shrink-1 notransition pa-0" :color="iconColor" style="width:auto; height:auto">{{ icon }}</v-icon>
                    <!-- </span> -->
                </div>
    ...
    methods: {
        ...
        updateIconSize() {
            ...
            while (low <= high && count < 21) {
                ...
                if(this.innerHeight(innerSpan) <= originalHeight){
                    size = mid;
                    low = mid + 1;
                } else {
                    high = mid - 1;
                ...
                console.log('Span final Height: ' + this.innerHeight(innerSpan));
