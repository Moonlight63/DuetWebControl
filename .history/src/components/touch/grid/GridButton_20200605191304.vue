            ...
            <div v-if="rows == 1" class="d-flex fill-height" style="width:100%;">
                <div ref="iconTest" class="fill-height">
                    <!-- <span ref="innerSpanIcon" class="iconFitted text-center d-inline-block"> -->
                        <v-icon ref="icon" size="1" class="mb-0 flex-shrink-1 notransition py-2" :color="iconColor" style="width:auto; height:auto">{{ icon }}</v-icon>
                    <!-- </span> -->
                </div>
    ...
    methods: {
        ...
        updateIconSize() {
            ...
            while (low <= high) {
                ...
                if((heightOnly || innerSpan.scrollWidth <= originalWidth) && (widthOnly || innerSpan.scrollHeight <= originalHeight)){
                    size = mid;
                    low = mid + 1;
                } else {
                    high = mid - 1;
