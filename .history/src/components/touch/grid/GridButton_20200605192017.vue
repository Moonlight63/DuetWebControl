<template>
    <grid-panel ref="elm" :columns='columns || 2' :rows='rows || 2' elevation="0">
        ...
        >
            ...
            <div v-if="rows == 1" class="d-flex fill-height my-2" style="width:100%;">
                <div ref="iconTest" class="fill-height">
                    <!-- <span ref="innerSpanIcon" class="iconFitted text-center d-inline-block"> -->
                        <v-icon ref="icon" size="1" class="mb-0 flex-shrink-1 notransition py-0 pr-2" :color="iconColor" style="width:auto; height:auto">{{ icon }}</v-icon>
                    <!-- </span> -->
                </div>
                <div ref="text" class="flex-grow-1 d-flex align-center text-center">
                    <span ref="innerSpan" style="font-size: 1px" class="textFitted text-center d-inline-block">
                        {{ label }}
                    </span>
                </div>
            </div>
    ...
    methods: {
        ...
        updateFontSize() {
            ...
            while (low <= high) {
                mid = (high + low) >> 1;
                innerSpan.style.fontSize = mid + 'px';
                if(innerSpan.clientWidth <= originalWidth && innerSpan.clientHeight <= originalHeight){
                    size = mid;
                    low = mid + 1;
                } else {
                    high = mid - 1;
