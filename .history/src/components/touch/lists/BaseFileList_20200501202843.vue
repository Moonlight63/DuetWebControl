...
<template>
	<div style="height: 100%">
		<v-data-table v-model="innerValue" v-bind="$props"
			...
			<template #item="props">
				<!-- <tr @click="onUpLevelClick()" @keydown.enter.prevent="onUpLevelClick()"
					@contextmenu.prevent=""	:data-filename="'*..'" draggable="false" tabindex="0">
				...
				<tr :active="props.isSelected" @keydown.space.prevent="props.select(!props.isSelected)"
					...
					<td v-for="header in props.headers" :key="header.value" :class="{ 'pr-0': header.value === 'data-table-select' }">
						<template v-if="header.value === 'data-table-select'">
							<v-checkbox v-if="props.item.name != '..'" :input-value="props.isSelected" @touchstart.stop="" @touchend.stop="" @click.stop.prevent="props.select(!props.isSelected)" primary hide-details class="mt-n1" tabindex="-1"></v-checkbox>
						</template>
						<template v-else-if="header.value === 'name'">
							<div class="d-inline-flex align-center">
								<slot :name="`${props.item.isDirectory ? 'folder' : 'file'}.${props.item.name}`">
									<v-icon class="mr-1">{{ props.item.isDirectory ? (props.item.name === ".." ? "mdi-folder-upload" : folderIcon) : fileIcon }}</v-icon> {{ props.item.name }}
								</slot>
							</div>
						</template>
						...
						<template v-else-if="header.unit === 'date'">
							{{ props.item.lastModified ? props.item.lastModified.toLocaleString() : (props.item.name === ".." ? "" : $t('generic.noValue')) }}
						</template>
...
export default {
	...
	methods: {
		...
		sort(items, sortBy, sortDesc) {
			...
			// THEN make sure directory up is first
			items.sort((a) => (a.name === "..") ? -1 : 0);
			return items;
		},
		async refresh() {
		...
		async loadDirectory(directory) {
			...
			try {
				...
				if (directory !== this.initialDirectory){
					files.unshift({
						isDirectory: true,
						name: "..",
						size: null,
		...
		onItemClick(props) {
			if (props.item.isDirectory) {
				if (props.item.name === "..") {
					let pathItems = this.innerDirectory.split('/').filter(item => item !== '');
					pathItems.pop();
					// let path = '';
					// pathItems.forEach(function(item) {
					// 	path = (path === '') ? item : path + '/' + item;
					// });
					this.loadDirectory(Path.combine(...pathItems));
				} else {
		...
		onUpLevelClick() {
			this.loadDirectory(Path.combine(this.innerDirectory, ".."));
		},
		setContextMenu(to) {
			this.contextMenu.shown = to;
		},
		onItemContextmenu(props, e) {
			if (this.contextMenu.shown || props.item.name === "..") {
				return;
			}
			this.onItemTouchEnd();
