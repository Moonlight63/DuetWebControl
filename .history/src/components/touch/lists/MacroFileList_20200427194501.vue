<template>
	<div class="component d-flex flex-column flex-grow-1" style="min-height: 0">
		...
		<touch-base-file-list ref="baselist" class="flex-grow-1" style="overflow: auto;" height="100%" fixed-header="true" ref="filelist" v-model="selection" :directory.sync="directory" :loading.sync="loading" sort-table="macros" @fileClicked="fileClicked" no-files-text="list.macro.noMacros">
...
export default {
	...
	methods: {
		...
		runFile(filename) {
			...
			this.$refs.baselist.onItemTouchEnd();
		}
	},
	mounted() {
