	...
	<v-dialog :value="shown" @input="$emit('update:shown', $event)" fullscreen hide-overlay transition="dialog-bottom-transition">
		<v-card class="d-flex flex-column">
			<v-app-bar flat dark color="primary" class="flex-grow-0 flex-shrink-1">
				...
				<v-btn dark text @click="setCarrotPos(5)">
					<v-icon class="mr-1">mdi-floppy</v-icon> test
				</v-btn>
			...
			<v-container class="py-0">
				<v-spacer></v-spacer>
				<touch-keyboard ref="keyboard" justify="center" align="center" @onChange="onKeyboardChange" @onKeyPress="onKeyPress" :input="innerValue" :theme="keyboardTheme" />			
				<v-spacer></v-spacer>
			</v-container>
		</v-card>
	</v-dialog>
...
export default {
	...
	methods: {
		...
		setCarrotPos(pos) {
			...
			console.log(this.$refs.textarea.$refs);
			console.log(this.$refs.textarea.$refs.input);
			console.log(this.$refs.textarea.$refs.input.setSelectionRange(3,3));
			console.log(this.$refs.textarea.$refs.input.selectionStart = pos);
			console.log(this.$refs.textarea.$refs.input.selectionEnd = pos);
			console.log(this.$refs.textarea.$refs);
		},
		async save() {
		...
		onKeyboardChange(input, caretPosition) {
			this.innerValue = input;
			this.valueChanged = true;
			console.log(caretPosition);
			this.$nextTick(() => {
				this.$refs.textarea.$refs.input.setSelectionRange(caretPosition,caretPosition)
			});
			//if (caretPosition !== null)
				//this.setCarrotPos(caretPosition);
		},
		onKeyPress(button) {
