	...
	<v-dialog v-model="shown" fullscreen persistent transition="dialog-bottom-transition">
		<v-card class="d-flex flex-column">
			<v-form ref="form" @submit.prevent="submit" class="flex-grow-1">
				...
				<v-card class="mx-auto mt-10" style="width: 50%">
					<v-card-text>
						{{ prompt }}

						<v-text-field ref="inputField" v-model="input" :rules="[v => !!v || $t('dialog.inputRequired'), v => !isNumericValue || isNumber(parseFloat(v)) || $t('dialog.numberRequired')]" required autofocus></v-text-field>
					</v-card-text>
				</v-card>
			</v-form>

			<v-container class="d-flex py-0 justify-center">
				<touch-keyboard ref="keyboard" @onChange="onKeyboardChange" @onKeyPress="onKeyPress" :input="input + ''" :theme="keyboardTheme" :numericOnly="isNumericValue" />			
			</v-container>
		</v-card>
	</v-dialog>
</template>
	...
	methods: {
		...
		onKeyboardChange(input, caretPosition) {
			...
			if (caretPosition !== null)
				this.$nextTick(() => {
					this.$refs.inputField.$refs.input.setSelectionRange(caretPosition,caretPosition)
				});
		},
		onKeyPress(button) {
