		...
		<v-card class="d-flex flex-column">
			<v-form ref="form" @submit.prevent="submit" class="flex-grow-1">
				<v-app-bar flat dark color="primary" class="flex-grow-0 flex-shrink-1">
					<!-- <v-btn text dark @click="hide">
						<v-icon>mdi-close</v-icon> {{ $t('generic.cancel') }}
					</v-btn>

					<v-spacer></v-spacer>
					<v-toolbar-title class="justify-center align-center">{{ title }}</v-toolbar-title>
					<v-spacer></v-spacer>
					...
					<v-row class="d-flex">
						<v-col class="d-flex flex-grow-1 justify-content-center">
						...
						<v-col class="d-flex flex-grow-1 justify-content-center">
							<v-toolbar-title class="justify-space-around">{{ title }}</v-toolbar-title>
						</v-col>
			...
			<v-container class="py-0">
				<v-spacer></v-spacer>
				<touch-keyboard @onChange="onKeyboardChange" @onKeyPress="onKeyPress" :input="innerValue" :theme="keyboardTheme" />			
				<v-spacer></v-spacer>
			</v-container>
		</v-card>
	</v-dialog>
