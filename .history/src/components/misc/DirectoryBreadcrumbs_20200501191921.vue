<template>
	<v-breadcrumbs :items="pathItems" divider=">">
		<template #item="dirs">
			<v-breadcrumbs-item href="javascript:void(0)" :disabled="dirs.item.disabled" @click="changeDirectory(dirs.item.href)" @dragover="dragOver(dirs.item.href, $event)" @drop.prevent="dragDrop(dirs.item.href, $event)">
				{{ dirs.item.text }}
			</v-breadcrumbs-item>
		</template>
	</v-breadcrumbs>
